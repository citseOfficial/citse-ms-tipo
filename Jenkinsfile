node {
   ...
   stage('Preparation') {
       git branch: 'main' , url: 'https://github.com/citseOfficial/citse-ms-tipo'
       mvnHome = tool 'MAVEN'
   }
   ...
   stage('Deployment') {
       try {
			sh 'chown jenkins:jenkins /var/lib/jenkins/workspace/citse-ms-tipo/runDeployment.sh'
			sh 'chmod +x /var/lib/jenkins/workspace/citse-ms-tipo/runDeployment.sh'
            sh '/var/lib/jenkins/workspace/citse-ms-tipo/runDeployment.sh'
       }catch (e){
